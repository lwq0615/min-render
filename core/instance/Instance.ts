...
import { Component, InstanceExpose, InstanceType, JsxNode, LIFE, RealDom, Refs, This, Watcher } from '../types/instance'
 ...
 */
export function createInstance(jsxNode: JsxNode, parentDom: RealDom, parentInstance?: Instance): Promise<Instance> {
...
export class Instance extends BaseInstance implements This {
    ...
    this.#renderTask = new Promise<void>((resolve) => {
      Promise.resolve().then(async () => {
        ...
        if (this.life >= LIFE.mounted) {
          ...
          this.childrens = await appendRealDomByJsxNode(childJsxNode, this.parentDom, this)
          // 第一次渲染的时候直接插入dom树
  ...
  useWatch(handler: Watcher['handler'], depends: Watcher['depends']) {
    ...
    if (oldWatcher) {
      ...
      } else if (!Object.is(oldWatcher.depends, depends)) {
        handler(oldWatcher.depends, depends)
      }
    }
