...
export class BaseInstance {
  constructor(jsxNode: JsxNode | string, parentDom: RealDom, parentInstance: Instance) {
  ...
  sameTypeAndKey(jsxNode: JsxNode | string) {
    ...
    if (
      ...
      return jsxNode.type === this.jsxNode.type && this.key === jsxNode.key && jsxNode.ref === this.jsxNode.ref
    }
    return false
  }
  ...
  // 重新渲染子元素
  async reRenderChildren(newJsxNodes: any): Promise<void> {
    ...
    for (const jsxNode of newJsxNodes) {
      // 有可重复使用的相同标签&&key节点
      const index: number = this.childrens.findIndex((instance) => instance.sameTypeAndKey(jsxNode))
      if (index !== -1) {
        ...
        children.push(
          ...(await appendRealDomByJsxNode(jsxNode, parentDom, this instanceof Instance ? this : this.parentInstance)),
        )
      }
    }
    // 卸载不需要的组件
